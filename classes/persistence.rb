...
class Persistance
  ...
  def load_genres
    return if File.empty?('./db/genres.json')
  ...
  def save_all(db_items)
    puts ' 😋 🤩 Saving Time.....................................'
    puts ' '
    puts 'Run:: 👉 Saving all Data______________________📚 🎵 🎮'
    puts ' '
    File.write('./db/books.json', JSON.pretty_generate(book_hashed(db_items['books'])), mode: 'w')
    File.write('./db/labels.json', JSON.pretty_generate(label_hashed(db_items['labels'])), mode: 'w')
    File.write('./db/musics.json', JSON.pretty_generate(music_albums_hashed(db_items['music_album'])), mode: 'w')
    File.write('./db/genres.json', JSON.pretty_generate(genre_hashed(db_items['genres'])), mode: 'w')
    File.write('./db/authors.json', JSON.pretty_generate(author_hashed(db_items['authors'])), mode: 'w')
    File.write('./db/games.json', JSON.pretty_generate(game_hashed(db_items['games'])), mode: 'w')
    puts 'Saving: 👉 75%'
